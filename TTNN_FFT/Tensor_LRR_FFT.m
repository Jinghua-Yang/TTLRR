function [ Z,E,U_rank,err ] = Tensor_LRR_FFT(X,A,Index,Data,max_iter,DEBUG,lambda,beta1,beta2,beta3,opts)
[n1,n2,n3]=size(X);
[~, n4, ~]=size(A);

rho = opts.rho;
max_beta = opts.max_beta;
% Phi  = fft(eye(n3))/sqrt(n3);
% PhiT = conj(Phi)';
...
%%?Pre compute
Ain = tt_inverse(A,beta1, beta2, beta3);
AH = tran(A);
while iter < max_iter
    iter = iter+1;
    ...
    Z_pre = Z;
    temp_Z = S - T/beta1;
    [Z, ~, U_rank] = prox_tnn(temp_Z, 1/beta1);
    ...
    J1 = beta1*Z + T + tprod(AH, beta3*M + N);
    J2 = beta2*E + H + beta3*M + N;
    temp_S = J1/beta3 - tprod(AH, J2)/(beta2+beta3);
    S = tprod(Ain, temp_S);
    G = J2/(beta2+beta3) - beta3*tprod(A, S)/(beta2+beta3);
